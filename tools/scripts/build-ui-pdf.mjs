...
// 1) 用 Playwright 截取 docs/design/prototypes/ui-mockup 下的核心页面（隐藏临时 mock-nav）
...
const MOCKUP = path.join(ROOT, "docs", "design", "prototypes", "ui-mockup");
...
async function main() {
  ...
  for (const p of PAGES) {
    await page.goto("file://" + path.join(MOCKUP, p.file), {
  ...
  await page.goto("file://" + path.join(MOCKUP, "create.html"), {
    waitUntil: "networkidle",
  });
  await page.addStyleTag({ content: hideMockNav });
  for (let k = 1; k <= 7; k++) {
    await page.evaluate((step) => {
      ...
      const title = document.querySelector(
        ".card:has(.stepper) .card-title .more",
      );
      if (title)
        title.textContent = `⚡ 全自动模式 · 当前执行：第 ${step} 步 / 共 7 步`;
    }, k);
  ...
  const flowRows = [
    ...
    [
      ...
      "合成",
      "任务中心 / 视频剪辑",
      "tasks.html · editor.html",
      "云端渲染合成，任务中心实时可视化进度，剪辑微调",
    ],
    [
      "⑥",
      "发布",
      "发布管理 / 账号管理 / 数据看板",
      "publish.html · accounts.html · analytics.html",
      "多平台分发排期，登录态巡检，数据回流反哺选题",
    ],
  ];

  const stepVariantImgs = Array.from(
    { length: 7 },
    (_, i) => `
...
<div class="page cover">
  ...
  <div class="meta">原型来源：docs/design/prototypes/ui-mockup（HTML 静态原型）<br>设计依据：docs/architecture/05-前端页面布局与全栈技术脚手架方案.md · docs/architecture/06-最终开发文档.md<br>生成时间：2026-07-19</div>
</div>
...
${moduleSections}
  ...
  await page.goto("file://" + path.join(OUT, "report.html"), {
    waitUntil: "networkidle",
  });
