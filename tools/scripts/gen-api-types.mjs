 ...
 *
 * 用法：
 *   node tools/scripts/gen-api-types.mjs                 # 从运行中的后端拉取 openapi.json 生成
 *   node tools/scripts/gen-api-types.mjs --file <path>   # 从本地 openapi 快照生成
 *   node tools/scripts/gen-api-types.mjs --check         # CI 零漂移校验：与现有产物不一致则退出码 1
 *
 * 产物：client/packages/types/src/generated.ts（每次后端发版重新生成，禁止手改）
 */
...
const OUT_FILE = resolve(ROOT, "client/packages/types/src/generated.ts");
const SNAPSHOT = resolve(ROOT, "server/contracts/openapi.snapshot.json");
const DEFAULT_URL =
...
function generate(spec) {
  ...
  return `/**
 * 由后端 OpenAPI 契约自动生成（tools/scripts/gen-api-types.mjs）
...
if (CHECK) {
  if (loadedFromRemote) {
    ...
    if (JSON.stringify(currentSnapshot) !== JSON.stringify(spec)) {
      console.error(
        "[gen-api] 契约漂移：server/contracts/openapi.snapshot.json 与运行中后端不一致，请执行 pnpm gen:api",
      );
      process.exit(1);
    }
  }
  ...
  if (strip(current) !== strip(output)) {
    console.error(
      "[gen-api] 契约漂移：client/packages/types/src/generated.ts 与后端 OpenAPI 不一致，请执行 pnpm gen:api",
    );
    process.exit(1);
  }
