...
async def send_message(db: AsyncSession, user_id: str, conversation_id: str, inp: SendMessageIn) -> MessageOut:
    ...
    sent = await im_provider.send_message(
        session=session,
    ...
    if not sent:
        raise HTTPException(
            ...
            detail={"code": "IM_SEND_FAILED", "message": "抖音私信发送失败，未写入本地记录"},
        )
    # 落库
...
async def start_listener(db: AsyncSession, user_id: str, inp: ListenerControlIn) -> ListenerStatusOut:
    ...
    account = await publish_repo.get_account(db, inp.accountId, user_id)
    if not account:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            detail={"code": "ACCOUNT_NOT_FOUND", "message": "发布账号不存在或不属于当前用户"},
        )
    if account.platform != "douyin":
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail={"code": "IM_PLATFORM_UNSUPPORTED", "message": "当前仅支持抖音私信监听"},
        )
    ...
    try:
        session = normalize_douyin_session(json.loads(account.session_json or "{}"))
    except (ValueError, json.JSONDecodeError) as exc:
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            detail={"code": "IM_SESSION_INCOMPLETE", "message": str(exc)},
        ) from exc
    if not session.get("device_id"):
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            detail={"code": "IM_SESSION_INCOMPLETE", "message": "旧登录态缺少私信连接参数，请重新扫码绑定抖音"},
        )
