...
from .schemas import (
    ConversationOut, ConversationPageOut, ListenerControlIn, ListenerStatusOut,
    MessageOut, MessagePageOut, RuleCreateIn, RuleOut, RuleUpdateIn, SendMessageIn,
)
...
def conv_to_out(c: IMConversation) -> ConversationOut:
    return ConversationOut(
        id=c.id, accountId=c.account_id, platform=c.platform,
        remoteUid=c.remote_uid, remoteNickname=c.remote_nickname,
        remoteAvatar=c.remote_avatar,
        lastMessageAt=c.last_message_at.astimezone(UTC).isoformat() if c.last_message_at else "",
        unreadCount=c.unread_count, status=c.status,
...
def msg_to_out(m: IMMessage) -> MessageOut:
    return MessageOut(
        ...
        msgType=m.msg_type, content=m.content, autoReplied=m.auto_replied,
...
def rule_to_out(r: IMAutoReplyRule) -> RuleOut:
    return RuleOut(
        ...
        triggerType=r.trigger_type, triggerPattern=r.trigger_pattern,
        replyMode=r.reply_mode, replyTemplate=r.reply_template,
        llmPrompt=r.llm_prompt, priority=r.priority,
        dailyLimit=r.daily_limit, delayMin=r.delay_min, delayMax=r.delay_max,
        enabled=r.enabled, createdAt=r.created_at.astimezone(UTC).isoformat())


# ============ 会话管理 ============

async def list_conversations(db: AsyncSession, user_id: str,
                             page: int = 1, page_size: int = 20) -> ConversationPageOut:
    items, total = await repo.list_conversations(db, user_id, page, page_size)
    return ConversationPageOut(
        items=[conv_to_out(c) for c in items], total=total, page=page, pageSize=page_size)


async def list_messages(db: AsyncSession, user_id: str, conversation_id: str,
                        page: int = 1, page_size: int = 50) -> MessagePageOut:
    ...
    return MessagePageOut(
        items=[msg_to_out(m) for m in items], total=total, page=page, pageSize=page_size)


async def send_message(db: AsyncSession, user_id: str, conversation_id: str,
                       inp: SendMessageIn) -> MessageOut:
    conv = await repo.get_conversation(db, conversation_id, user_id)
    if not conv:
        raise HTTPException(status.HTTP_404_NOT_FOUND,
                            detail={"code": "NOT_FOUND", "message": "会话不存在"})
    ...
    session = await _load_account_session(conv.account_id)
    ...
    msg = await repo.create_message(
        db, conversation_id=conv.id, user_id=user_id,
        direction="out", msg_type=inp.msg_type, content=content_json)
...
async def create_rule(db: AsyncSession, user_id: str, inp: RuleCreateIn) -> RuleOut:
    rule = await repo.create_rule(
        db, user_id=user_id, account_id=inp.accountId, name=inp.name,
        trigger_type=inp.triggerType, trigger_pattern=inp.triggerPattern,
        reply_mode=inp.replyMode, reply_template=inp.replyTemplate,
        llm_prompt=inp.llmPrompt, priority=inp.priority,
        daily_limit=inp.dailyLimit, delay_min=inp.delayMin,
        delay_max=inp.delayMax, enabled=inp.enabled)
    return rule_to_out(rule)


async def update_rule(db: AsyncSession, user_id: str, rule_id: str,
                      inp: RuleUpdateIn) -> RuleOut:
    ...
    field_map = {
        "name": "name", "triggerType": "trigger_type", "triggerPattern": "trigger_pattern",
        "replyMode": "reply_mode", "replyTemplate": "reply_template",
        "llmPrompt": "llm_prompt", "priority": "priority", "dailyLimit": "daily_limit",
        "delayMin": "delay_min", "delayMax": "delay_max", "enabled": "enabled"}
...
async def get_listener_status(db: AsyncSession, user_id: str) -> list[ListenerStatusOut]:
    ...
    result = []
    for s in states:
        result.append(ListenerStatusOut(
            accountId=s.account_id, platform="douyin", status=s.status,
...
async def start_listener(db: AsyncSession, user_id: str, inp: ListenerControlIn) -> ListenerStatusOut:
    ...
    await repo.upsert_listener_state(
        db, account_id=inp.accountId, user_id=user_id,
        status="listening", started_at=datetime.now(UTC), error_msg="")
    await emit(CHANNEL_IM, {"kind": EV_LISTENER_STATUS, "userId": user_id,
                            "accountId": inp.accountId, "status": "listening"})
...
async def stop_listener(db: AsyncSession, user_id: str, inp: ListenerControlIn) -> ListenerStatusOut:
    ...
    await repo.upsert_listener_state(
        db, account_id=inp.accountId, user_id=user_id,
        status="disconnected", error_msg="")
    await emit(CHANNEL_IM, {"kind": EV_LISTENER_STATUS, "userId": user_id,
                            "accountId": inp.accountId, "status": "disconnected"})
    return ListenerStatusOut(accountId=inp.accountId, status="disconnected")


# ============ 消息接收处理（供 Worker 调用） ============

async def handle_incoming_message(account_id: str, user_id: str, remote_uid: str,
                                  remote_nickname: str, remote_avatar: str,
                                  msg_type: int, content: str,
    ...
    async with _session() as db:
        ...
        if conv is None:
            conv = await repo.create_conversation(
                db, account_id=account_id, user_id=user_id, platform="douyin",
                remote_uid=remote_uid, remote_nickname=remote_nickname,
                remote_avatar=remote_avatar,
                dy_conversation_id=dy_conversation_id,
                dy_conversation_short_id=dy_conversation_short_id,
                dy_ticket=dy_ticket,
                last_message_at=datetime.now(UTC), unread_count=1)
        else:
            conv.last_message_at = datetime.now(UTC)
        ...
        msg = await repo.create_message(
            db, conversation_id=conv.id, user_id=user_id,
            direction="in", msg_type=msg_type, content=content_json)

        # 3. 推送前端
        await emit(CHANNEL_IM, {
            "kind": EV_NEW_MESSAGE, "userId": user_id,
...
async def _try_auto_reply(db: AsyncSession, conv: IMConversation, text: str) -> None:
    ...
    delay = random.uniform(
        max(0, matched_rule.delay_min),
        max(matched_rule.delay_min, matched_rule.delay_max))
    asyncio.create_task(_delayed_reply(
        account_id=conv.account_id, user_id=conv.user_id,
        conversation_id=conv.id, platform=conv.platform,
        dy_conversation_id=conv.dy_conversation_id,
        dy_conversation_short_id=conv.dy_conversation_short_id,
        dy_ticket=conv.dy_ticket,
        reply_text=reply_text, rule_id=matched_rule.id, delay=delay))


async def _delayed_reply(*, account_id: str, user_id: str, conversation_id: str,
                         platform: str, dy_conversation_id: str,
                         dy_conversation_short_id: str, dy_ticket: str,
                         reply_text: str, rule_id: str, delay: float) -> None:
    """#4 独立 Task：延迟后发送回复并落库"""
    await asyncio.sleep(delay)
    try:
        im_provider = registry.im_driver(platform)
        session = await _load_account_session(account_id)
        await im_provider.send_message(
            session=session,
    ...
    async with _session() as db:
        reply_msg = await repo.create_message(
            ...
            direction="out", msg_type=7,
            content=json.dumps({"text": reply_text}, ensure_ascii=False),
            auto_replied=True, rule_id=rule_id)
    ...
    # 推送前端
    await emit(CHANNEL_IM, {
        "kind": EV_AUTO_REPLIED, "userId": user_id,
        "conversationId": conversation_id,
        "reply": reply_text, "messageId": reply_msg.id})
...
async def _generate_reply(db: AsyncSession, rule: IMAutoReplyRule,
                          incoming_text: str, conv: IMConversation) -> str:
...
async def _load_account_session(account_id: str) -> dict:
    """#1 异步加载账号 session"""
    from app.modules.publish import repository as pub_repo
    async with SessionLocal() as db:
        acc = await pub_repo.get_account(db, account_id)
        if acc:
            return json.loads(acc.session_json or "{}")
        return {}
