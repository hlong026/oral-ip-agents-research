...
async def resolve_provider_avatar_id(db: AsyncSession, user_id: str, avatar_id: str) -> str:
    ...
    if a.status != "ready":
        raise HTTPException(status.HTTP_400_BAD_REQUEST,
                            detail={"code": "NOT_READY", "message": "数字人尚未就绪"})
    return a.provider_avatar_id or avatar_id


async def handle_avatar_callback(db: AsyncSession, task_id: str, status_code: int,
                                 avatar_id_remote: str) -> None:
...
def to_out(a: Avatar) -> AvatarOut:
    return AvatarOut(
        ...
        avatarType=a.avatar_type, scene=a.scene, style=a.style,
        coverUrl=f"/media/{a.cover_key}" if a.cover_key else None,
        previewUrl=f"/media/{a.preview_key}" if a.preview_key else None,
        status=a.status, createdAt=a.created_at.astimezone(UTC).isoformat(),
    )
