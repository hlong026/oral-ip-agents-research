...
def stop_listening(account_id: str) -> None:
    ...
    if conn:
        ...
        def log_close_error(close_task: asyncio.Future) -> None:
            error = close_task.exception()
            if error:
                logger.debug("conn.close error: %s", error)

        t.add_done_callback(log_close_error)
...
async def _listen_loop(account_id: str, user_id: str) -> None:
    ...
    # 加载账号 session
    async with SessionLocal() as db:
        account = await pub_repo.get_account(db, account_id, user_id)
        if not account:
            logger.error(f"[IMWorker] owned account {account_id[:8]} not found")
            return
        user_id = account.user_id
        session = pub_repo.account_session(account)
