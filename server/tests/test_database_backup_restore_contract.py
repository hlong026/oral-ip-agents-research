...
def test_pg_environment_keeps_password_out_of_connection_arguments() -> None:
    url = make_url(
        "postgresql+asyncpg://oral:super-secret@staging-db.internal:5432/"
        "oral_staging?sslmode=require"
    )
...
def test_restore_wrapper_is_staging_only_and_stops_workers_before_pg_restore() -> None:
    script = (ROOT / "deploy/scripts/restore-database-staging.sh").read_text(
        encoding="utf-8"
    )
    ...
    assert script.index("compose stop worker server") < script.index(
        "database_restore_staging"
    )
    assert script.index("database_restore_staging") < script.index(
        "--profile migration run --rm --no-deps migration"
    )
    assert script.index("--profile migration run --rm --no-deps migration") < script.index(
...
def test_backup_wrapper_supports_prod_backup_but_never_restore() -> None:
    ...
    restore = (ROOT / "deploy/scripts/restore-database-staging.sh").read_text(
        encoding="utf-8"
    )
    ...
    assert "DEPLOY_ENVIRONMENT=production" not in restore
