 ...
 * 视频剪辑为流水线第⑥步不占一级位；设置入口收进算力卡 ⚙，不设 side-foot。
 */
  ...
  return [
    {
      ...
      items: [
        ...
        { to: "/tasks", label: "任务中心", icon: "☰", badge: taskActive, badgeTone: "brand" },
      ],
    },
    {
      title: "资产",
    ...
    },
    {
      title: "分发",
      items: [
        { to: "/publish/jobs", label: "发布管理", icon: "⇪", badge: publishFailed, badgeTone: "danger" },
    ...
    <div className="relative">
      ...
      >
        <span
          ...
          style={{ background: current?.avatarGrad || "linear-gradient(135deg,#6366F1,#22D3EE)" }}
        >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block truncate text-xs text-text-3">{current?.domain ?? "—"}</span>
        </span>
  ...
  return (
    <div className="glass px-3.5 py-3">
      <div className="flex items-center justify-between">
        ...
        <NavLink to="/settings" title="算力设置" className="text-xs text-text-3 transition-colors hover:text-text-1">
          ⚙
        </NavLink>
      </div>
      <div className="mt-1 text-grad text-sm font-bold">{balance.toLocaleString()}</div>
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/5">
        <div className="h-full rounded-full bg-brand-grad-x transition-all" style={{ width: `${pct}%` }} />
      </div>
      <div className="mt-1.5 flex items-center justify-between text-[11px] text-text-3">
        <span>剩余 {pct}%</span>
        <NavLink to="/account" className="text-brand-to hover:underline">
          充值
        </NavLink>
      </div>
    </div>
...
export default function Sidebar() {
  ...
  const nav = buildNav((running?.total ?? 0) + (waiting?.total ?? 0), failedJobs?.total ?? 0);
                  ...
                  {item.badge ? (
                    <span
                      className={`ml-auto rounded-full px-1.5 py-px text-[10px] font-bold ${
                        item.badgeTone === "danger" ? "bg-danger/20 text-danger" : "bg-brand-from/25 text-brand-to"
                      }`}
                    >
