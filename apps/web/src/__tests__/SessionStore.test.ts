...
const SESSION_STORE_MODULE = "../../../../packages/api-client/src/session-store";
...
describe("session-store", () => {
  ...
  it("非 Tauri 环境：读返回 null、写为 no-op", async () => {
    const { loadPersistedRefreshToken, persistRefreshToken } = await loadStore();
  ...
  it("Tauri 壳：写非空令牌走 session_token_write，清空走 session_token_clear", async () => {
    ...
    expect(invoke).toHaveBeenCalledWith("session_token_write", { token: "rt-2" });
