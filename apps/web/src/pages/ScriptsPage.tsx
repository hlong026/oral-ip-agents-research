...
function NewScriptPanel({ onClose }: { onClose: () => void }) {
  ...
  const fail = (e: unknown, fallback: string) => setError(e instanceof HttpError ? e.body.message : fallback);
  ...
  const extract = async (file?: File) => {
    ...
    setError("");
    try {
      const res = await contentApi.parse(file ? undefined : url.trim(), file);
      if (res.scriptId) {
  ...
  const genTopics = async () => {
    ...
    setError("");
    try {
      const res = await contentApi.topics(topic.trim());
  ...
  const genScript = async (picked: string) => {
    ...
    setError("");
    try {
      const rw = await contentApi.rewrite(`请围绕选题「${picked}」生成 60 秒口播文案`, "theme");
      const s = await contentApi.createScript({ title: picked, text: rw.text, platform: "topic", topic: picked });
  ...
  const saveManual = async () => {
    ...
    setError("");
    try {
      const s = await contentApi.createScript({ text: text.trim(), platform: "manual" });
      navigate(`/scripts/${s.id}`);
    } catch (e) {
      fail(e, "保存失败，请重试");
    } finally {
    ...
    <div className="glass-strong space-y-4 p-5">
      ...
      {tab === "url" && (
        <div className="space-y-3">
          ...
          />
          <button className="btn-primary w-full" disabled={busy || !url.trim()} onClick={() => void extract()}>
            {busy ? "提取中…" : "提取文案 →"}
          </button>
        </div>
      )}
      ...
      {tab === "topic" && (
        <div className="space-y-3">
          <div className="flex gap-2">
            ...
            />
            <button className="btn-ghost shrink-0" disabled={busy || !topic.trim()} onClick={() => void genTopics()}>
              生成选题
            </button>
          </div>
          {topics.length > 0 && (
            <div className="space-y-2">
              <div className="text-xs text-text-3">选一个选题，AI 直接成稿并入库：</div>
              {topics.map((t) => (
                <button
                  key={t}
                  disabled={busy}
      ...
      {tab === "script" && (
        <div className="space-y-3">
          ...
          />
          <button className="btn-primary w-full" disabled={busy || text.trim().length < 10} onClick={() => void saveManual()}>
            {busy ? "保存中…" : "保存到文案工坊 →"}
          </button>
        </div>
      )}
...
export default function ScriptsPage() {
  const { data, isLoading } = useQuery({ queryKey: ["scripts"], queryFn: () => contentApi.scripts() });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-text-3">解析转写、AI 仿写、去重检测的全部文案资产</p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {(data ?? []).map((s) => (
          <ScriptCard key={s.id} script={s} onOpen={() => navigate(`/scripts/${s.id}`)} />
        ))}
      </div>
      ...
      {!isLoading && (data ?? []).length === 0 && !showNew && (
        <div className="glass py-16 text-center text-text-3">
          还没有文案资产，
          <button className="text-brand-to hover:underline" onClick={() => setShowNew(true)}>
            从链接/选题开始 →
          </button>
        </div>
      )}
      {!isLoading && (data ?? []).length === 0 && (
        <div className="text-center text-xs text-text-3">
          ...
          <Link to="/create?step=link" className="text-brand-to hover:underline">
            去一键成片 →
          </Link>
        </div>
      )}
...
function ScriptCard({
  ...
  script: { id: string; title: string; platform: string; rewrittenText: string; originalText: string; similarityScore: number; createdAt: string };
  onOpen: () => void;
}) {
  const rel = useRelativeTime(script.createdAt);
  const sim = script.similarityScore;
  const simColor = sim < 30 ? "text-success border-success/40" : sim < 60 ? "text-warning border-warning/40" : "text-danger border-danger/40";
  const text = script.rewrittenText || script.originalText;

  return (
    <button onClick={onOpen} className="glass card-hover flex flex-col p-4 text-left">
      <div className="mb-2 flex items-center justify-between gap-2">
        <span className="min-w-0 flex-1 truncate text-sm font-medium">{script.title || "未命名文案"}</span>
        <span className={`chip shrink-0 ${simColor}`}>去重 {sim.toFixed(0)}</span>
      </div>
      <p className="line-clamp-3 flex-1 text-sm leading-relaxed text-text-2">{text}</p>
