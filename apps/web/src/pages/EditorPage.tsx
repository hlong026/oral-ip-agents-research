...
type AsrWord = { word: string; start: number; end: number };
...
function isAsrWord(value: unknown): value is AsrWord {
...
function readAsrWords(value: unknown): AsrWord[] {
  return Array.isArray(value) ? value.filter(isAsrWord) : [];
}

/** 从 ASR words 构建字幕片段（使用真实时间戳） */
function buildSubtitlesFromAsrWords(words: AsrWord[]): SubtitleSegment[] {
  ...
  let currentStart = Math.max(0, Math.round(firstWord.start * 1000)); // ms，四舍五入避免浮点误差
  let currentEnd = Math.max(0, Math.round(firstWord.end * 1000));
  let currentText = firstWord.word;
...
function defaultContent(task: PipelineTask): PublicationContentSpec {
  ...
  // 尝试从 ASR words 构建字幕
  const asrWords = readAsrWords(task.artifacts?.words);
  ...
  if (asrWords.length > 0) {
    ...
    subtitleSegments = (lines.length ? lines : [task.title]).map(
      (text, index) => ({
        id: `segment-${index + 1}`,
        startMs: index * 1800,
        endMs: (index + 1) * 1800,
  ...
  return {
    ...
    subtitles: {
      enabled: true,
      source: asrWords.length > 0 ? "asr_aligned" : "manual",
...
export default function EditorPage() {
  ...
  useEffect(() => {
    ...
    const element = document.getElementById(
      `subtitle-segment-${activeSegmentIndex}`,
    );
    if (element) {
