  ...
  return (
    <section className="glass p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-text-3">
            用户超出设备上限（当前 {limit}
            台）后自动提交申请；批准时选择要解绑的旧设备，新设备下次登录自动绑定。
          </p>
        </div>
        <select
      ...
      <div className="mt-4 space-y-4">
        {(data?.items ?? []).map((item) => (
          <div key={item.id} className="rounded-lg border border-stroke/60 p-4">
            ...
            {item.status === "pending" && (
              <div className="mt-3 space-y-2">
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  >
                    {unbindChoice[item.id] ? "批准并解绑所选" : "批准"}
                  </button>
                  <button
                    className="btn-ghost"
      ...
      {tab === "devices" ? (
        ...
        <div className="grid gap-5 xl:grid-cols-[420px_1fr]">
          ...
          <section className="glass p-5">
            ...
            <p className="mt-1 text-sm text-text-3">
              明文码为五组五位字符（如 KTNPV-98W6R-8Q3M2-Y47CG-PDKV3）；CSV
              仅在生成后提供一次，请立即保存。
            </p>
            {generate.data ? (
              <div className="mt-4">
